...
最後に，これらの結果をもとに簡単な制御系を構築し，脚ロボット(シミュレーション)で走行動作を実現する．~~ （申し訳ありませんが，分量を考慮して削除しました．）
...
- ~~単純なロボット脚モデル（２リンクアーム）を用いて，アーム先端の位置制御ならびにSLIPモデルの仮想的再現をおこなう~~ （申し訳ありませんが，分量を考慮して削除しました．）
